...
use crate::{
    ...
    ffi::{
        ...
        future::{
            ...
            future::{
                mongoc_future_destroy, mongoc_future_get_insert_one, mongoc_future_is_finished,
            },
            insert::mongoc_future_insert_one,
        },
        ops::insert::InsertOneResult,
...
fn test_insert_one_null_document() {
    ...
    unsafe {
        ...
        let future = mongoc_future_insert_one(
            client,
            ptr::null(),
            db.as_ptr(),
            coll.as_ptr(),
        ...
        let mut result_out: *const InsertOneResult = ptr::null();
        let mut error_out: *mut Error = ptr::null_mut();
        let ok = mongoc_future_get_insert_one(future, &mut result_out, &mut error_out);
        assert!(!ok);
